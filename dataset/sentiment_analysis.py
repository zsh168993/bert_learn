...
class CLSDataset(Dataset):
    def __init__(self, corpus_path, vocab, max_seq_len, data_regularization=False):
    ...
    def __getitem__(self, item):
        ...
        text_input = [self.vocab.sos_index] + text_input + [self.vocab.eos_index]
        # 如果序列的长度超过self.max_seq_len限定的长度, 则截断
        text_input = text_input[:self.max_seq_len]
        padding = [self.vocab.pad_index for _ in range(self.max_seq_len - len(text_input))]
        text_input.extend(padding)
    ...
    def tokenize_char(self, segments):
        return [self.vocab.stoi.get(char, self.vocab.unk_index) for char in segments]